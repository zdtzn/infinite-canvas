...
while [ "$attempt" -le "$attempts" ]; do
  if docker pull "${IMAGE_REPOSITORY}:latest" >/dev/null 2>&1; then
    image_revision="$(docker image inspect -f '{{index .Config.Labels "org.opencontainers.image.revision"}}' "${IMAGE_REPOSITORY}:latest" 2>/dev/null || true)"
    if [ "$image_revision" = "$EXPECTED_COMMIT" ]; then
      break
    fi
  fi
  if [ "$attempt" -eq "$attempts" ]; then
    echo "Timed out waiting for ${IMAGE_REPOSITORY}:latest to publish commit ${EXPECTED_COMMIT}; current revision is '${image_revision}'" >&2
    exit 1
  fi
  echo "Waiting for image commit ${EXPECTED_COMMIT} (${attempt}/${attempts})"
