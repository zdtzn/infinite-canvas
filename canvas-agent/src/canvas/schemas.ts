...
export const toolInputSchemas = {
    ...
    workbench_image_generate: z.object({ prompt: z.string(), model: z.string().optional(), resolution: z.string().optional(), quality: z.string().optional(), outputFormat: z.string().optional(), size: z.string().optional(), count: z.number().optional(), run: z.boolean().optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    workbench_image_generate: "在生图工作台填入提示词并按需设置 model、resolution（输出分辨率）、quality（生成质量）、outputFormat、size 和 count。先调用 workbench_image_get_config 获取当前模型实际支持的选项。run 默认 true，会自动跳转并触发生成；提交后可用 generation_get_status 查询状态。",
