...
export const CONFIG_DIR = resolveConfigDir(process.env.CANVAS_AGENT_CONFIG_DIR);
export const CONFIG_FILE = path.join(CONFIG_DIR, "canvas-agent.json");
export const VERSION = readPackageVersion();
export const AGENT_PROMPT = fs.readFileSync(new URL("../agent-instructions.md", import.meta.url), "utf8");
const initializedWorkspaces = new Set<string>();

export function resolveConfigDir(configDir?: string) {
    return configDir?.trim() ? path.resolve(configDir) : path.join(os.homedir(), ".infinite-canvas");
}
