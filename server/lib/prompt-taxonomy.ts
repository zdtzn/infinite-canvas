...
export const PROMPT_TAXONOMY_REVISION = "single-theme-v1";
...
export function classifyPromptTags(
  input: PromptTaxonomyInput,
): PromptTaxonomyTag[] {
  ...
  const scored = TAXONOMY_RULES.map((rule, index) => {
    const prominentMatches = rule.keywords.filter((keyword) =>
      includesKeyword(prominentText, keyword),
    ).length;
    const bodyMatches = rule.keywords.filter((keyword) =>
