...
export function openAiApiUrl(config: ManagedAiConfig, path: string) {
    return isServerManagedConfig(config) ? `/api/ai/${encodeURIComponent(config.channelId)}/openai/${path.replace(/^\/+/, "")}` : withLocalProxy(buildApiUrl(config.baseUrl, path));
}
...
export function geminiApiBase(config: ManagedAiConfig) {
    ...
    return withLocalProxy(/\/(?:v1|v1beta)$/i.test(normalized) ? normalized : `${normalized}/v1beta`);
}
