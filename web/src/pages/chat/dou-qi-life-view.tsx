...
import { Archive, Backpack, BookOpen, ChevronRight, CirclePlus, Compass, Dice5, Eye, Flame, Heart, MessageCircle, Move, Save, ScrollText, Send, Shield, Sparkles, Swords, Trash2, UserRound, WandSparkles } from "lucide-react";
...
function ActionDock({ state, suggestions, sending, draft, onDraftChange, onSend }: { state: DouQiLifeState; suggestions: DouQiLifeSuggestion[]; sending: boolean; draft: string; onDraftChange: (value: string) => void; onSend: (action?: string) => void }) {
    ...
    const cultivationActions = !state.battle.active
        ...
        : [];
    return <div className="mt-3 shrink-0 border-t border-stone-200/70 pt-3 dark:border-white/10"><div className="mb-2 flex flex-wrap gap-2">{(battleActions.length ? battleActions : cultivationActions).map(([label, action]) => <Button key={label} size="small" disabled={sending} icon={battleActions.length ? (label === "移动" ? <Move className="size-3.5" /> : <Swords className="size-3.5" />) : <WandSparkles className="size-3.5" />} onClick={() => onSend(action)}>{label}</Button>)}{state.battle.active ? <Button size="small" type="dashed" disabled={sending} onClick={() => onSend("我自由决定下一步战斗行动")}>自由行动</Button> : <Button size="small" type="dashed" disabled={sending} icon={<Eye className="size-3.5" />} onClick={() => onSend("我先观察眼前的环境，再决定下一步行动。")}>自由行动</Button>}{suggestions.map((item) => <Button key={item.id} size="small" disabled={sending} onClick={() => onSend(item.action)}>{item.label}</Button>)}</div><div className="flex items-end gap-2"><Input.TextArea value={draft} onChange={(event) => onDraftChange(event.target.value)} disabled={sending} autoSize={{ minRows: 1, maxRows: 4 }} maxLength={4_000} placeholder={state.battle.active ? "描述你的战斗行动，状态由天地裁定" : "写下你要做的事，世界会回应结果"} onPressEnter={(event) => { if (!event.shiftKey) { event.preventDefault(); onSend(); } }} /><Button type="primary" icon={<Send className="size-4" />} disabled={!draft.trim() || sending} loading={sending} onClick={() => onSend()}>行动</Button></div></div>;
}
