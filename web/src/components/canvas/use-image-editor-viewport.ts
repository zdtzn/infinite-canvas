...
export function useImageEditorViewport(image: ImageSize | null, open: boolean) {
    ...
    const setZoomAround = useCallback(
        (nextZoom: number, clientX?: number, clientY?: number) => {
            ...
            zoomAnchorRef.current = {
                zoom: boundedZoom,
                ratioX: clamp((pointerX - stageRect.left) / Math.max(1, stageRect.width), 0, 1),
                ratioY: clamp((pointerY - stageRect.top) / Math.max(1, stageRect.height), 0, 1),
                viewportX: pointerX - viewportRect.left,
                viewportY: pointerY - viewportRect.top,
            };
            setZoom(boundedZoom);
        },
        [baseSize.height, baseSize.width, zoom],
    ...
    return {
        ...
        stageStyle: { left: stageOffset.left, top: stageOffset.top, width: stageSize.width, height: stageSize.height } satisfies CSSProperties,
        mediaStyle: {
