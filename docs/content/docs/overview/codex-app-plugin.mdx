...
```text
请从 https://github.com/zdtzn/infinite-canvas.git 安装 Infinite Canvas Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/zdtzn/infinite-canvas.git ~/plugins/infinite-canvas
```
...
插件会启动本地 Canvas Agent，读取连接地址和 token。当前插件说明中的 `https://canvas.best/` 是上游项目提供的托管站点，并不是本仓库的生产地址；使用当前二次开发版本时，应打开自己的部署地址，或明确要求插件使用本地项目。
...
`@basketikun/canvas-agent` 目前仍是上游发布到 npm 的真实包名。当前仓库没有发布 `@zdtzn/canvas-agent`，因此不要改写这条命令。
